...
code(
    """# @title 11b) Waist-up keepers (face LoRA only, no retraining)
import os
import torch
from IPython.display import display
from PIL import Image, ImageDraw
...
if "pipe" not in globals():
    raise RuntimeError("Run cell 11 first so a clean model is loaded.")
...
print("This cell does NOT retrain.")
print("Face LoRA only. Same waist-up look as the 3 similar frames.")
...
seeds = [707, 2025, 1301, 42, 314, 8192, 12345, 33333]
print("Prompt:", prompt)
print("Seeds:", seeds)
...
for seed in seeds:
    ...
    path = os.path.join(OUTPUT_DIR, "preview_waist_keeper_" + str(seed) + ".png")
    image.save(path)
    images.append(image)
    paths.append(path)
    print("Saved:", path)
...
for idx, tile in enumerate(labeled):
    ...
    y = (idx // 4) * thumb_h
    sheet.paste(tile, (x, y))
sheet_path = os.path.join(OUTPUT_DIR, "preview_waist_KEEPER_SHEET.png")
sheet.save(sheet_path)
print("WAIST-UP KEEPERS (face LoRA only):")
display(sheet)
print("Saved:", sheet_path)
upload_project_file(sheet_path)
for path in paths:
